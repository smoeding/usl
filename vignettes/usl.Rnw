...
\usepackage{Sweave}
\SweaveOpts{engine=R, eps=FALSE, keep.source=FALSE, height=3.8}
...
<<rtbarplot>>
@
\caption{Rate of efficiency per processor for different numbers of processors running the raytracing software}
\label{fig:rtbarplot}
\end{figure}
...
plotted to indicate the fitted scalability function.
...
See \autoref{fig:ciplot1} for the entire plot.
...
<<ciplot1>>
@
\caption{The result of the SPEC SDM91 benchmark with confidence bands for the scalability function at the 95\% level}
\label{fig:ciplot1}
\end{figure}
...
increased further. Again this is an effect of \emph{diminishing returns} as
...
<<ovplot1>>
@
\caption{Components of the execution time for parallelized workloads of the SPECSDM91 benchmark. The time is measured as a fraction of the time needed for serial execution of the workload.}
\label{fig:ovplot1}
\end{figure}
...
time. For higher degrees of parallelism the impact of coherency delays has a
growing impact. Also observe that due to contention and coherency effects the
big difference in ideal execution time between $100$ and $200$ parallel jobs
effectively leads to no improvement in the total execution time.
