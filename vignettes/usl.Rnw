...
\usepackage{Sweave}
\SweaveOpts{engine=R, eps=FALSE, keep.source=FALSE, height=3.5}
...
Bootstrapping is used to calculate the confidence intervals for the parameters.
They must be taken with a grain of salt since the level of accuracy of the
original measurements is usually unknown. Depending on the type of the
requested bootstrap confidence interval the function might not be able to
produce a valid result at all.
...
The \kbd{predict} function can also be used to calculate a confidence interval
...
\autoref{fig:ciplot1} shows the final plot.
...
<<ciplot1>>
@
\caption{The result of the SPEC SDM91 benchmark with confidence intervals for the scalability function at the 95\% level}
\label{fig:ciplot1}
\end{figure*}
