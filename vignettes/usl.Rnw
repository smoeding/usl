...
We can solve this nonlinear equation to estimate the coefficients $\alpha$
...
formula to represent the missing point of reference (\cite{Gun:usl3}).
...
\item Finally the residual standard error estimates how well the model fits
  the data. We can see that the difference between the model prediction and
  the measured values is typically within $\Sexpr{signif(sigma(usl.model), 3)}$
...
estimate the confidence intervals. This has been changed since bootstrapping
