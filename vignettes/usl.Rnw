...
mathematical grounding has been published by Dr.~Gunther (\cite{Gun07}).
...
$\beta = 0$ (cf. \cite{Gun07}). Therefore the Universal Scalability Law can
be seen as a generalization of Amdahl's Law for speedup in parallel
computing.
...
different loads $N_i$. Initially the Unversal Scalabilty Law normalized the
measurements using the throughput $X(1)$ for load $1$ as the point of
reference (cf. \cite{Gun07}). This placed an additional burden on the
performance analyst who needed to either measure the value or find a smart
way to estimate it.
...
percent is quiete a good value for a reasonable complex application. Note that
