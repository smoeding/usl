...
The model coefficients $\alpha$ and $\beta$ can be retrieved with the
\kbd{coef()} function.
...
\item[\texttt{nls}:] This method uses the \kbd{nls()} function of the stats
  ...
  coefficients $\alpha$ and $\beta$ but also the scale factor for the
...
--- could reduce $\beta$ by half and want to predict how the scalability of
...
\Sexpr{signif(peak.scalability(usl.model, beta=0.00005), 3)} if we would be
