...
\item[\texttt{nlxb}:] A nonlinear regression model is also used in this case.
  ...
  from the nlmrt package (see \cite{R:nlmrt}). This method also estimates
...
\item[\texttt{db\_time}:] The average time spent inside the database either
  ...
  exactly one second will contribute a total of two seconds per second.
  Oracle has coined the term \emph{Average Active Sessions} (AAS) for this
  metric.
\end{labeling}
