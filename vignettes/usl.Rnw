...
opts_chunk$set(fig.width=7, fig.height=4.0, fig.align='center')
...
<<ovplot1,fig.keep='none'>>=
barplot(height = t(ovhd), names.arg = load[, 1], xlab = names(load),
         legend.text = TRUE)
@
...
<<ovplot1>>
@
\caption{Decomposition of the execution time for parallelized workloads of the SPECSDM91 benchmark. The time is measured as a fraction of the time needed for serial execution of the workload.}
\label{fig:ovplot1}
\end{figure}
