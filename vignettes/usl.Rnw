...
the transaction rate in \cref{fig:oraplot1}. It shows the familiar pattern
...
small uncertainty about the calculated coefficients.
...
live Oracle database system to learn about the scalability. Note that neither
the Oracle software nor the application needed any additional instrumentation
to collect this data. Also the analysis was done without any internal knowledge
...
% could argue that the estimated function seems to be too pessimistic. There are
...
%' <<echo=FALSE, fig.cap='Foo'>>=
%' p <- with(oracledb, data.frame(db_time, txn_rate))
%' 
%' # Plot all points
%' plot(txn_rate ~ db_time, p, xlab = "Average active sessions", ylab = "Transactions per second")
%' 
