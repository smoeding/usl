...
\Cref{eqn:usl2} has the Universal Scalability Law where $C(N) = X(N) / X(1)$
...
In \cite{Gun07} Dr.~Gunther proves that \cref{eqn:usl2} is reduced to Amdahl's
Law for $\beta = 0$. Therefore the Universal Scalability Law can be seen as
a generalization of Amdahl's Law for speedup in parallel computing.

We could solve this nonlinear equation to estimate the coefficients $\alpha$
and $\beta$ using a sparse set of measurements for the throughput $X_i$ at
different loads $N_i$. The computations used to solve the equation for the
measured values are discussed in \cite{Gun07}.

Initially the Unversal Scalabilty Law normalized the measurements using the
throughput $X(1)$ for load $1$ as the point of reference. This placed an
additional burden on the performance analyst who needed to either measure the
value or find a smart way to estimate it.

It was then discovered that a third coefficient $\gamma$ can be added to the
formula to represent the missing point of reference. \Cref{eqn:usl3} shows the
updated Universal Scalability Law using three parameters.

\begin{equation}\label{eqn:usl3}
X(N) = \frac{\gamma N}{1 + \alpha (N - 1) + \beta N (N - 1)}
\end{equation}

A nonlinear regression will solve this equation and find best-fit values for
all three parameters $\alpha$, $\beta$ and $\gamma$.
...
scalability is analyzed. Up to version 1.8.0 the \usl{} package always used
\texttt{sigma} and \texttt{kappa} as coefficients. This has been changed
starting with the 2.0.0 release of the \usl{} package. Now the package will
also use \texttt{alpha} and \texttt{beta} as coefficient names to follow
Dr.~Gunther's nomenclature.
...
\item The third coefficient $\gamma$ estimates the throughput for a single
  ...
  measurement is also caused by the regression.
\item Finally $R^2$ estimates how well the model fits the data. We can see
  that the model is able to explain about
...
The model coefficients can be retrieved with the \kbd{coef()} function.

<<>>=
coef(usl.model)
@

We can see that the model estimates the contention parameter to be about
$\Sexpr{signif(100 * coef(usl.model)[['alpha']], 2)}$ percent and the coherency
parameter to be $\Sexpr{signif(coef(usl.model)[['beta']], 2)}$.

With $\beta = 0$ the model indicates no coherency penalty for the system. In
this case the throughput will asymptotically approach an upper bound $X_{max}$
as the number of processors is increased. This bound can be calculated using
\cref{eqn:amdahl}.

\begin{equation}\label{eqn:amdahl}
X_{max} = \gamma \alpha^{-1}
\end{equation}

The model predicts a limit of
$\Sexpr{signif((1/coef(usl.model)[['alpha']]) * coef(usl.model)[['gamma']], 3)}$
as the upper bound for the maximal achievable throughput. According to the
model there is no way to increase the throughput that number, no matter how
many processors are available whatsoever.

The corresponding confidence intervals\footnotemark{} for the model coefficients
are returned by calling the \kbd{confint()} function.

\footnotetext{Earlier releases of the \usl{} package used bootstrapping to
estmate the confidence intervals. This has been changed since bootstrapping
with a small sample size may not give the desired accuracy. Now the confidence
...
The confidence interval shows the range that the true value is expected lies in
for the given confidence level. We can see that the true $\beta$ might in fact
not really be exactly zero. One reason is that all measurements contain
measurement/observation errors. The other reason is that such a small $\beta$
might only cause a measuable effect when a much higher number of processors are
used.
...
\item[\texttt{nlxb}:] A nonlinear regression model is also used in this case.
  ...
  from the nlsr package (see \cite{R:nlsr}). It is expected to be more robust
...
\item[\texttt{default}:] The default method traditionally used a transformation
  ...
  introduced in chapter 5.2.3 of \cite{Gun07}. Starting with version 2.0.0 of
  the \usl{} package this algorithm is no longer available. The \kbd{nlxb()}
...
\Sexpr{signif(coef(usl.model)[['beta']], 1)}. The parameter $\alpha$
indicates that about \Sexpr{signif(100*coef(usl.model)[['alpha']], 2)}
